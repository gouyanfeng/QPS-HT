...
public class TogglePowerHandler : IRequestHandler<TogglePowerCommand, bool>
{
    ...
    public async Task<bool> Handle(TogglePowerCommand request, CancellationToken cancellationToken)
    {
        ...
        var command = request.PowerOn ? room.DeviceConfig.PowerOnCommand : room.DeviceConfig.PowerOffCommand;
        await _mqttService.SendCommandAsync(room.DeviceConfig.MqttTopic, command);
        return true;
    }
}